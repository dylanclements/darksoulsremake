...
Bonfires are now associated with the location they are placed upon. 

3 new classes and 2 new interfaces were added to expand upon Bonfires in this game.

Classes:
- BonfireTeleportAction: moves player from one bonfire location to another
- BonfireNetwork: A singleton. Stores a list of all active bonfires. Generates teleport actions.
- BonfireLightAction: Allows BonfireRestAction and adds Bonfire to BonfireNetwork

Interfaces:
- BonfireSpawn: Actors implement this if they spawn at bonfires, meaning their spawn is subject to change based on the last bonfire they rested at.
- IBonfire: Bonfires implement this to give Bonfire actions access to Bonfire data.

**Resulting UML relationships**:
- Association between Bonfire and Location to indicate that Bonfires now store the location they are placed upon.
- Associations drawn between all Bonfire related actions and IBonfire, as these all store an IBonfire instance to interact with Bonfire data.
- Dependencies drawn to any bonfire class that gets instantiated in another.
- Generalisations drawn from Bonfire actions to Action.

## Fog Doors

Fog Doors are Ground objects that allow the player to travel from one adjacent location to another. Therefore, FogDoors
will know 2 locations, the front and the back. The player can travel either from the front location to the back or 
vice versa using EnterFogDoorAction which stores 1 location, whichever location is the final location for the player.

**Resulting UML relationships**:
- Association drawn between FogDoor and Location with multiplicity 1-2. For each FogDoor, 2 locations are stored. the front and the back of the door.
- Association drawn between EnterFogDoorAction and Location with multiplicity 1-1. EnterFogDoor will transport the actor to that location.
- Dependency drawn from FogDoor to EnterFogDoorAction. EnterFogDoorAction is generated by FogDoor.
- Generalisations drawn from FogDoor/EnterFogDoorAction to Ground/Action.

## Aldrich

The Aldrich fight introduces ranged weapons to this game (see other changes section).
DarkmoonLongbow is Aldrich's weapon and it implements RangedWeapon. Its range is 3 as per the requirements.
Aldrich implements ```isWithinRange``` and returns true when the ```targetLocation``` is within the DarkmoonLongbow's range.
Aldrich will be able to shoot at opponents from up to 3 squares away.

**Resulting UML relationships**:
- Realisations drawn from Aldrich to Resettable, Soul, Aggressor and ActorStatus.
- Generalisations drawn from Aldrich to LordOfCinder, DarkmoonLongbow to RangedWeapon, RangedWeapon to WeaponItem and CindersOfALord to PortableItem.
- Dependencies drawn from Aldrich to DarkmoonLongbow because Aldrich instantiates a DarkmoonLongbow in its constructor and adds it to inventory.
- Dependencies drawn from Aldrich to CindersOfALord because Aldrich instantiates a CindersOfALord in its constructor and adds it to inventory. 
- Association drawn from Aldrich to Behaviour because Aldrich will store some sort of behaviour at all times.
...
it allows us to override `allowableActions()` method which is an easy way to prompt 
...
By creating a level of abstraction between high-level modules, 
